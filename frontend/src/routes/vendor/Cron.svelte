	...
	function generateCron() {
		...
		if ("daily" === tabShow) {
			if (cronSetts.daily.dailyOpt === "everyDay") {
				value = `${cronSetts.daily.everyDay.seconds} ${cronSetts.daily.everyDay.minutes} ${hourToCron(
			...
			if (cronSetts.daily.dailyOpt === "everyWeekDay") {
				value = `${cronSetts.daily.everyWeekDay.seconds} ${cronSetts.daily.everyWeekDay.minutes} ${hourToCron(
		...
		if ("weekly" === tabShow) {
			const days = Object.keys(cronSetts.weekly)
				...
				.join(",");
			value = `${cronSetts.weekly.seconds} ${cronSetts.weekly.minutes} ${hourToCron(
		...
		if ("monthly" === tabShow) {
			if ("specificDay" === cronSetts.monthly.monthlyOpt) {
				value = `${cronSetts.monthly.specificDay.seconds} ${cronSetts.monthly.specificDay.minutes} ${hourToCron(
					cronSetts.monthly.specificDay.hours,
					cronSetts.monthly.specificDay.hourType,
				)} ${cronSetts.monthly.specificDay.day} 1/${cronSetts.monthly.specificDay.month} ? *`;
			}
			if ("specificWeekDay" === cronSetts.monthly.monthlyOpt) {
				value = `${cronSetts.monthly.specificWeekDay.seconds} ${cronSetts.monthly.specificWeekDay.minutes} ${hourToCron(
		...
		if ("yearly" === tabShow) {
			if ("specificMonthDay" === cronSetts.yearly.yearlyOpt) {
				value = `${cronSetts.yearly.specificMonthDay.seconds} ${cronSetts.yearly.specificMonthDay.minutes} ${hourToCron(
					cronSetts.yearly.specificMonthDay.hours,
					cronSetts.yearly.specificMonthDay.hourType,
				)} ${cronSetts.yearly.specificMonthDay.day} ${cronSetts.yearly.specificMonthDay.month} ? *`;
			}
	...
	function parseCron() {
		...
		if (segments.length === 6 || segments.length === 7) {
			...
			if (cron.match(/\d+ 0\/\d+ \* 1\/1 \* \? \*/)) {
				...
				dayOfWeek.split(",").forEach((weekDay) => (cronSetts.weekly[weekDay] = true));
				const parsedHours = parseInt(hours);
...
<div>
	{#each tabs as tab}
		{#if showAdvanced || (!showAdvanced && tab !== "advanced")}
			<button class={tab === tabShow ? "active" : ""} on:click={() => changeTab(tab)}>{tab}</button>
		{/if}
	{/each}
</div>
<hr />
...
{#if showAdvanced && tabShow === "advanced"}
	<div class="advanced">
		<input type="text" bind:value={cronSetts.advanced} on:change={generateCron} />
	</div>
{/if}
...
{#if tabShow === "yearly"}
	<div class="yearly">
		<div>
			...
			/>
			Every
			<select bind:value={cronSetts.yearly.specificMonthDay.month} on:change={() => generateCron()}>
				{#each monthNames as item}
					<option value={item.key}>{item.val}</option>
				{/each}
			</select>
			on the
			<select bind:value={cronSetts.yearly.specificMonthDay.day} on:change={() => generateCron()}>
				{#each monthlyDays as item}
					<option value={item.key}>{item.val}</option>
				{/each}
			</select>
			at
			<select bind:value={cronSetts.yearly.specificMonthDay.hours} on:change={() => generateCron()}>
				{#each hours as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			<select bind:value={cronSetts.yearly.specificMonthDay.minutes} on:change={() => generateCron()}>
				{#each minutes as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			{#if showSeconds}
				<select bind:value={cronSetts.yearly.specificMonthDay.seconds} on:change={() => generateCron()}>
		...
		<div>
			...
			/>
			On the
			<select bind:value={cronSetts.yearly.specificMonthWeek.whichWeekDay} on:change={() => generateCron()}>
				{#each weekDaysNo as item}
					<option value={item.key}>{item.val}</option>
				{/each}
			</select>
			<select bind:value={cronSetts.yearly.specificMonthWeek.weekDay} on:change={() => generateCron()}>
				{#each Object.entries(weekDaysMap) as [key, val]}
					<option value={key}>{val}</option>
				{/each}
			</select>
			of
			<select bind:value={cronSetts.yearly.specificMonthWeek.month} on:change={() => generateCron()}>
				{#each monthNames as item}
					<option value={item.key}>{item.val}</option>
				{/each}
			</select>
			at
			<select bind:value={cronSetts.yearly.specificMonthWeek.hours} on:change={() => generateCron()}>
				{#each hours as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			<select bind:value={cronSetts.yearly.specificMonthWeek.minutes} on:change={() => generateCron()}>
				{#each minutes as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			{#if showSeconds}
				<select bind:value={cronSetts.yearly.specificMonthWeek.seconds} on:change={() => generateCron()}>
					{#each seconds as item}
						<option value={item}>{item}</option>
					{/each}
				</select>
...
{#if tabShow === "monthly"}
	<div class="monthly">
		<div>
			...
			/>
			On the
			<select bind:value={cronSetts.monthly.specificDay.day} on:change={() => generateCron()}>
				{#each monthlyDays as item}
					<option value={item.key}>{item.val}</option>
				{/each}
			</select>
			of every
			<select bind:value={cronSetts.monthly.specificDay.month} on:change={() => generateCron()}>
				{#each months as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			month(s) at
			<select bind:value={cronSetts.monthly.specificDay.hours} on:change={() => generateCron()}>
				{#each hours as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			<select bind:value={cronSetts.monthly.specificDay.minutes} on:change={() => generateCron()}>
				{#each minutes as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			{#if showSeconds}
				<select bind:value={cronSetts.monthly.specificDay.seconds} on:change={() => generateCron()}>
					{#each seconds as item}
						<option value={item}>{item}</option>
					{/each}
				</select>
		...
		<div>
			...
			/>
			On the
			<select bind:value={cronSetts.monthly.specificWeekDay.whichWeekDay} on:change={() => generateCron()}>
				{#each weekDaysNo as item}
					<option value={item.key}>{item.val}</option>
				{/each}
			</select>
			<select bind:value={cronSetts.monthly.specificWeekDay.weekDay} on:change={() => generateCron()}>
				{#each weekDaysShort as item}
					<option value={item}>{weekDaysMap[item]}</option>
				{/each}
			</select>
			of every
			<select bind:value={cronSetts.monthly.specificWeekDay.month} on:change={() => generateCron()}>
				{#each months as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			at
			<select bind:value={cronSetts.monthly.specificWeekDay.hours} on:change={() => generateCron()}>
				{#each hours as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			<select bind:value={cronSetts.monthly.specificWeekDay.minutes} on:change={() => generateCron()}>
				{#each minutes as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			{#if showSeconds}
				<select bind:value={cronSetts.monthly.specificWeekDay.seconds} on:change={() => generateCron()}>
					{#each seconds as item}
						<option value={item}>{item}</option>
					{/each}
				</select>
...
{#if tabShow === "weekly"}
	<div class="weekly">
		{#each weekDaysShort as item}
			<label>
				<input type="checkbox" value={item} bind:checked={cronSetts.weekly[item]} on:change={() => generateCron()} />
				{weekDaysMap[item]}
			</label>
		{/each}
		<div>
			Start time
			<select bind:value={cronSetts.weekly.hours} on:change={() => generateCron()}>
				{#each hours as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			<select bind:value={cronSetts.weekly.minutes} on:change={() => generateCron()}>
				{#each minutes as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			{#if showSeconds}
				<select bind:value={cronSetts.weekly.seconds} on:change={() => generateCron()}>
					{#each seconds as item}
						<option value={item}>{item}</option>
					{/each}
				</select>
...
{#if tabShow === "daily"}
	<div class="daily">
		<div class="entity">
			...
			/>
			Every
			<select bind:value={cronSetts.daily.everyDay.days} on:change={() => generateCron()}>
				{#each days as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
		...
		<div class="entity">
			at
			<select bind:value={cronSetts.daily.everyDay.hours} on:change={() => generateCron()}>
				{#each hours as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			<select bind:value={cronSetts.daily.everyDay.minutes} on:change={() => generateCron()}>
				{#each minutes as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			{#if showSeconds}
				<select bind:value={cronSetts.daily.everyDay.seconds} on:change={() => generateCron()}>
					{#each seconds as item}
						<option value={item}>{item}</option>
					{/each}
				</select>
		...
		<div>
			...
			<select bind:value={cronSetts.daily.everyWeekDay.hours} on:change={() => generateCron()}>
				{#each hours as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			<select bind:value={cronSetts.daily.everyWeekDay.minutes} on:change={() => generateCron()}>
				{#each minutes as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
			{#if showSeconds}
				<select bind:value={cronSetts.daily.everyWeekDay.seconds} on:change={() => generateCron()}>
					{#each seconds as item}
						<option value={item}>{item}</option>
					{/each}
				</select>
...
{#if tabShow === "hourly"}
	<div class="hourly">
		<div class="entity">
			Every
			<select bind:value={cronSetts.hourly.hours} on:change={() => generateCron()}>
				{#each hours as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
		...
		<div class="entity">
			on minute
			<select bind:value={cronSetts.hourly.minutes} on:change={() => generateCron()}>
				{#each minutes as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
		...
		{#if showSeconds}
			<div class="entity">
				and second
				<select bind:value={cronSetts.hourly.seconds} on:change={() => generateCron()}>
					{#each seconds as item}
						<option value={item}>{item}</option>
					{/each}
				</select>
...
{#if tabShow === "minutes"}
	<div class="minutes">
		<div class="entity">
			Every
			<select bind:value={cronSetts.minutes.minutes} on:change={() => generateCron()}>
				{#each minutes as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
		...
		{#if showSeconds}
			<div class="entity">
				on second
				<select bind:value={cronSetts.minutes.seconds} on:change={() => generateCron()}>
					{#each seconds as item}
						<option value={item}>{item}</option>
					{/each}
				</select>
...
{#if tabShow === "seconds"}
	<div class="seconds">
		<div>
			Every
			<select bind:value={cronSetts.seconds.seconds} on:change={() => generateCron()}>
				{#each seconds as item}
					<option value={item}>{item}</option>
				{/each}
			</select>
